...
import ChatPage from "./components/custom/ChatPage.jsx"; // ← ADDED
import ChatIcon from "./components/custom/ChatIcon.jsx"; // ← ADDED
...
function App() {
  ...
  useEffect(() => {
    ...
    // Location info
    const locationEl = locationInfoRef?.current?.querySelector(".location");
    const carousel = locationInfoRef?.current?.querySelectorAll(".carousel");
    const locationInfo =
      locationInfoRef?.current?.querySelector(".location-info");
    if (locationEl && carousel && locationInfo) {
      timeline
        .from(locationEl, { opacity: 0, y: 100 })
      ...
      <div className="app tracking-tighter min-w-[320px]">
        ...
        <div className="container max-w-[1024px] w-full min-w-[320px] h-auto">
          <Routes>
            ...
            <Route
              path="/chat"
              element={isAuthenticated ? <ChatPage /> : <Hero />}
            /> {/* ← NEW ROUTE */}
          </Routes>
        </div>
